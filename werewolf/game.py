...
class GameMaster:
    ...
    def __init__(
        ...
        Args:
          ...
          num_threads: Number of worker threads to use for player actions.
        """
        self.state = state
    ...
    def _get_bid(self, player_name: str) -> Tuple[int, Any]:
        ...
        if bid is None:
            raise ValueError(
                f"{player_name} did not return a valid bid. Find the raw response in the `bid` field in the log",
            )
        if bid > 1:
    ...
    def run_summaries(self) -> None:
        ...
        with ThreadPoolExecutor(max_workers=self.num_threads) as executor:
            player_summaries = {
                name: executor.submit(self.state.players[name].summarize) for name in self.this_round.players
            }
    ...
    def run_voting(self) -> Tuple[Dict[str, str], List[VoteLog]]:
        ...
        with ThreadPoolExecutor(max_workers=self.num_threads) as executor:
            player_votes = {name: executor.submit(self.state.players[name].vote) for name in self.this_round.players}
    ...
    def exile(self) -> None:
        ...
        if self.this_round.exiled is not None:
            ...
            announcement = f"The majority voted to remove {exiled_player} from the game."
        else:
            announcement = "A majority vote was not reached, so no one was removed from the game."
        ...
        while not self.state.winner:
            ...
            for name in self.this_round.players:
                if self.state.players[name].gamestate:
                    self.state.players[name].gamestate.round_number = self.current_round_num + 1
