...
class GameView:
    ...
    def remove_player(self, player_to_remove: str) -> None:
        ...
        if player_to_remove not in self.current_players:
            print(
                f"Player {player_to_remove} not in current players: {self.current_players}",
            )
...
class Player(Deserializable):
    ...
    def _get_game_state(self) -> Dict[str, Any]:
        ...
        remaining_players = [
            f"{player} (You)" if player == self.name else player for player in self.gamestate.current_players
        ]
    ...
    def _generate_action(
        ...
        result_key, allowed_values = (
            (action, options) if action in ["vote", "remove", "investigate", "protect", "bid"] else (None, None)
        )
    ...
    def from_json(cls, data: Dict[Any, Any]) -> "Player":
        ...
        o.gamestate = data.get("gamestate")
...
class Werewolf(Player):
    ...
    def eliminate(self) -> Tuple[Optional[str], "LmLog"]:
        ...
        options = [
            player for player in self.gamestate.current_players if player not in {self.name, self.gamestate.other_wolf}
        ]
        random.shuffle(options)
...
class Seer(Player):
    ...
    def unmask(self) -> Tuple[Optional[str], LmLog]:
        ...
        options = [
            ...
            if player != self.name and player not in self.previously_unmasked
        ]
        random.shuffle(options)
...
class Round(Deserializable):
    ...
    def from_json(cls, data: Dict[Any, Any]) -> "Round":
        ...
        o.eliminated = data.get("eliminated")
        o.unmasked = data.get("unmasked")
        o.protected = data.get("protected")
        o.exiled = data.get("exiled")
...
class State(Deserializable):
    ...
    def __init__(
        ...
        self.players: Dict[str, Player] = {
            player.name: player for player in self.villagers + self.werewolves + [self.doctor, self.seer]
        }
...
class VoteLog(Deserializable):
    ...
    def from_json(cls, data: Dict[Any, Any]) -> "VoteLog":
        ...
        log = LmLog.from_json(data.get("log"))
...
class RoundLog(Deserializable):
    ...
    def from_json(cls, data: Dict[Any, Any]) -> "RoundLog":
        ...
        investigate = data.get("investigate")
        ...
        for votes in data.get("votes", []):
            ...
            v_logs.extend(VoteLog.from_json(v) for v in votes)

        for r in data.get("bid", []):
            r_logs = []
            o.bid.append(r_logs)
            r_logs.extend((player[0], LmLog.from_json(player[1])) for player in r)
